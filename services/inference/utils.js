...
import {
  initializeSuperRes,
  multiUpscale,
} from "@/services/inference/upscaling";
...
export function prepareImage(imageArray, model) {
  ...
  if (model === "superRes") {
    const tensor = new ort.Tensor("uint8", imageArray.data.slice(), [
      width,
      height,
      4,
    ]);
    ...
    const tensor = new ort.Tensor("uint8", newND.data.slice(), [
      1,
      3,
      height,
      width,
...
export async function fetchModel(
  ...
  while (true) {
    ...
    if (done) {
      ...
      setProgress(
        startProgress + (received / length) * (endProgress - startProgress)
      );
    }
  }
  return data.buffer;
